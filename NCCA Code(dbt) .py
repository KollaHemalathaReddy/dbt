def hello_world(request):
    ...
    bucket_name = 'exporttobq2'
    variables_file_name = 'variable_file.txt'
    ...
    api_to_GCS_bucket=Variables_dict['api_to_GCS_bucket']
    api_gcs_destination_folder = Variables_dict["api_gcs_destination_folder"]
    api_filename=Variables_dict['api_filename']
    api_source_files_backup_bucket=Variables_dict['api_source_files_backup_bucket']
    api_bq_tables_backup_bucket=Variables_dict['api_bq_tables_backup_bucket']
    api_to_BQ_dataset=Variables_dict['api_to_BQ_dataset']
    api_source_files_list=[]
    api_count_of_source_files=[]
    api_destination_tables_list=[]
    api_count_of_destination_tables=[]
    api_distinct_count_of_destination_tables=[]
    

    def create_json(json_object, api_filename):
        '''
        this function will create json object in
        google cloud storage
        '''
        # create a blob
        bucket = storage_client.get_bucket(api_to_GCS_bucket)
        blob = bucket.blob("{}/{}".format(api_gcs_destination_folder,api_filename))
        ...
        result = api_filename + ' upload complete'
        return {'response' : result}
    
    print("file created successfully")

    def api_source_count(gcs_bucket_name,gcs_folder_name,project_name,api_source_files_list,api_count_of_source_files):
        ...
        for blob in bucket.list_blobs(prefix=gcs_folder_name):
            ...
            if blob.name.endswith('json'):  #Checking for json files in the gcs folder
                ...
                api_source_files_list.append(filenametest)
                api_count_of_source_files.append(len(df))
                print(len(df))

    print("src cnt success")
        ...
        for filename in filenames:
            if(pathlib.Path(filename).suffix ==source_file_type):
                ...
                if source_file_type=='.csv':
                    a = pd.read_csv("/tmp/"+filename, encoding_errors= 'ignore', low_memory=False, encoding='UTF-8')
    ...
    #function to move files from FTP_json to GCS to bigquery
    def api_to_bq_full_load():
        logger.debug('api Fullload job started')

        bucket = storage_client.get_bucket(api_to_GCS_bucket)

        source_files_backup_to_gcs_folder(api_to_GCS_bucket,api_source_files_backup_bucket,api_gcs_destination_folder,'json')    #function call to backup the source files from ftp server(json files)
        print('ftp_json source_files_backup method success')

        #get_ftp(FTP_HOST, FTP_USER, FTP_PASS,'.json',FTP_json_to_GCS_bucket,FTP_json_gcs_destination_folder,FTP_json_source_files_list,FTP_json_count_of_source_files) #function call to load the files(json) from ftp server to gcs bucket
        #print('data successfully loaded from ftp_json to gcs')

        create_json(json_object, api_filename)
        print('api file creation success')

        api_source_count(api_to_GCS_bucket,api_gcs_destination_folder,gcpprojectname,api_source_files_list,api_count_of_source_files)
        print("src count success of api")

        bq_create_dataset(bigquery_client,api_to_BQ_dataset)  #function call---to create the dataset dynamically
        print('api create_dataset method success')

        bqtables_backup_to_gcs_folder(gcpprojectname,api_to_BQ_dataset,api_bq_tables_backup_bucket,'NEWLINE_DELIMITED_JSON') #function call---to backup the bigquery tables to gcs folder before truncating
        print('api-bq_tables_backup method success')

        gcs_to_bigquery(api_to_GCS_bucket,api_gcs_destination_folder,gcpprojectname,api_to_BQ_dataset,api_destination_tables_list,api_count_of_destination_tables)   #function call---to upload the files from gcs bucket to bigquery tables
        print('api-gcs_to_bq method success')

        source_destination_count_validation_function(api_source_files_list,api_destination_tables_list,api_count_of_source_files,api_count_of_destination_tables) #function call--to validate source and destination count and upload related data to bq validation table
        print('api-count method success')

        logger.debug('api Fullload job successfully completed')

        insert_logging_info_to_bqtable(mysql_logfile_path,gcpprojectname,validation_nullvalues_dataset,bq_logging_info_table) #function call--to insert the log data to bigquery logging_info_table
        print('api logging method success')

        print("data successfully loaded from api to bq")
